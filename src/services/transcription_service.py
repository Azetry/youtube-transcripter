...
Acquisition (H2/H3): this host runs structured YouTube extraction; on failure the
fallback policy surfaces an operator-visible decision (no remote full-pipeline
delegation).
"""
...
class AcquisitionOutcome:
    ...
    continues. On failure, ``fallback_decision`` describes the recommended
    operator-visible next step (retry, manual, abort, etc.).
    """
...
class TranscriptionService:
    ...
    def _acquire(self, url: str, progress: Callable) -> AcquisitionOutcome:
        """Run the structured acquisition phase.

        1. Try this-host strategies via H2 ThisHostAcquisitionService.
        2. On failure, consult H3 fallback policy and return diagnostics.
        """
